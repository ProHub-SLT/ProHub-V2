...
namespace PROHUB.Data
{
    ...
    // ---------------------------------------------------------
    public interface IExternalSolutionsActivitiesService
    {
    ...
    // ---------------------------------------------------------
    // 2. THE EXTERNAL IMPLEMENTATION
    // ---------------------------------------------------------

    public class ExternalSolutionsActivitiesDataAccess : IExternalSolutionsActivitiesService
    {
        private readonly string _connectionString;

        public ExternalSolutionsActivitiesDataAccess(IConfiguration configuration)
        {
        ...
        public async Task<List<ProjectActivity>> GetAllAsync(string search = null, string sortColumn = null, string sortOrder = null, int? filterPlatformId = null)
        {
            ...
            string query = @"
                SELECT pa.*, 
                       COALESCE(mp.Platforms, ep.Platform_Name) AS PlatformName,
                       COALESCE(esol.Platform_Name, ep.Platform_Name) AS SolutionName,
            ...
            {
                conditions.Add("pa.Platform_ID = @FilterPlatformId");
            }
            ...
            if (filterPlatformId.HasValue)
            {
                command.Parameters.AddWithValue("@FilterPlatformId", filterPlatformId.Value);
            }
        ...
        public async Task<List<ExternalPlatform>> GetExternalSolutionsAsync()
        {
            ...
            try
            {
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    {
                        Id = reader.GetInt32(reader.GetOrdinal("ID")),
                        PlatformName = reader.GetString(reader.GetOrdinal("Platform_Name"))
                    });
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error retrieving external solutions: {ex.Message}");
                throw;
            }
            return list;
        }
